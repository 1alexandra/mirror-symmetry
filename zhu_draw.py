...
def draw_contour(
    ...
    if method == 'plt':
        ...
        if axis_marker and axis_point and axis_vec:
            ...
            plt.plot([np.real(s1), np.real(s2)],
                     [np.imag(s1), np.imag(s2)],
        ...
        cnt = zc.u_to_cnt(1j * h + np.conjugate((u + vec) * scale))
        cv2.drawContours(img, [cnt], 0, 255, line_w)
        if axis_point and axis_vec:
            new_v = np.conjugate(axis_vec)
            new_p = 1j * h + np.conjugate((axis_point + vec) * scale)
            s1, s2 = line_in_rect(new_p, new_v, 0, w, 0, h)
...
def draw_hull(u, u_h, h, m):
    ...
    draw_contour('plt', h,
                 ...
                 point_marker='y-',
                 edge_color='yellow')
    draw_contour('plt', u_h,
                 label='points to search axis',
                 point_marker='b+',
                 edge_color=None)
    centroid = u.mean()
    plt.plot([np.real(centroid)], [np.imag(centroid)],
             'yo', label='centroid')
    plt.grid()
    plt.legend()
